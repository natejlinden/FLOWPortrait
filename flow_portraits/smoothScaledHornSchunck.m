...
function [x, y, u, v, sub_data] = smoothScaledHornSchunck(data, history_delay, alpha_hs, max_iter, avg_win, smooth_win, alpha_smooth)
    % Syntax:
    % [x, y, u, v, sub_data] = smoothScaledHornSchunk(data, alpha_hs, max_iter, history_delay, avg_win, smooth_win, alpha_smooth)
    ...
    % sub_data -     shortened data matrix 
        ...
        % Run scaling code
        if history_delay % case with history scaling
        ...
        else % case with no time history scaling
        u_w = u;
        v_w = v;
        sub_data = data; 
        end
