...
% This script recreates all the panes in Figure 3
...
for clip = 1:num_clips
    ...
    % show image
    dat = ftle.f(:,:,idx);
    normalize = [-1*max(max(abs(dat))), 0.85*max(max(abs(dat)))];
